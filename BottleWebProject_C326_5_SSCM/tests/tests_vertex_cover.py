...
class TestVertexCoverSolver(unittest.TestCase):
    """Тесты для алгоритма поиска минимального вершинного покрытия"""
    ...
    # ТЕСТ 7: Длинная чередующаяся цепь
...
class TestVertexCoverValidation(unittest.TestCase):
    ...
    # ТЕСТ 11: Проверка корректности индексов рёбер
    ...
    # ТЕСТ 12: Обнаружение дубликатов рёбер
    ...
    # ТЕСТ 13: Проверка на рёбра внутри одной и той же доли (нарушение двудольности)
    ...
    def test_empty_fractions_with_edges(self):
        """TC-15: Списки вершин пустые, но переданы рёбра"""
        U = []
        V = []
        edges = [(1, 2)]
        
        has_invalid_context = len(edges) > 0 and (len(U) == 0 or len(V) == 0)
        self.assertTrue(has_invalid_context, "Рёбра ссылаются на несуществующие доли")

    # ТЕСТ 16: Отрицательные индексы вершин
    def test_negative_vertex_indices(self):
        """TC-16: Проверка на отрицательные вершины в рёбрах"""
        edges = [(-1, 4), (2, 5)]
